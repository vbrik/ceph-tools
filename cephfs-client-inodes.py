...
def load_live(rank, match_type=None, match_val=None):
    ...
    print(f'warning: querying `client ls` on {rank_desc} -- '
          f'this can be resource-intensive on a busy cluster', file=sys.stderr)
...
def main():
    ...
    parser.add_argument('-r', '--rank', type=int, default=None, metavar='RANK',
    ...
    if args.file is None:
        ...
        if args.rank is not None:
            print('error: --rank only applies in live mode (omit FILE to use it)', file=sys.stderr)
            return 1
