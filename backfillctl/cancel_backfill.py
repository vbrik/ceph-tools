...
def cancel_whole_pg(
    ...
    is, there are no companions: _close_pins only confirms the result is valid,
    ...
    if is_ec:
        ...
        if not pins:
            return [], skipped
        closed, why = _close_pins(up, acting, pins, osd_host)
        moves = []
        if why is None:
    ...
    for _, to_osd in pairs:
        for other in new_up:
            if other != to_osd and _same_place(to_osd, other, osd_host):
